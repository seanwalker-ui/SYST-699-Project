...
# -------------------------------------------------------------------------------------------------------- #
    ...
    def compute_action_towards_target(self, drone_pos, target_pos):
        ...
        norm = np.linalg.norm(direction_vector)    # Normalize the direction vector
       
        
        if norm > 1e-3:  # Avoid division by zero and tiny movements
            # Scale the movement speed by increasing the step size
            speed_factor = 10                                               # Adjust for faster/slower movement
            direction_vector = (direction_vector / norm) * speed_factor     #  Scale by speed_factor
    ...
    # Modify the reward calculation based on distance to the target
    def compute_reward(self, drone_positions, target_pos):
        # Reward based on how close drones are to the target
        total_reward = 0
        distances_to_target = []
        
        for pos in drone_positions:
            # Calculate the distance between the drone and the target
            distance = np.linalg.norm(np.array(pos[:2]) - np.array(target_pos))
            distances_to_target.append(distance)
            
            # Encourage drones to move closer by penalizing large distances
            total_reward -= distance  # Negative reward for being far
        
        # Encourage efficiency by providing extra reward if all drones are close enough
        if all(dist < 20 for dist in distances_to_target):  # 5-pixel threshold
            total_reward += 100  # Large positive reward for reaching the target
...
# --------------------------------------------------------------------------------------------------------- #      
        # Compute the new reward based on proximity to the target
        reward = self.compute_reward(drone_positions, target_pos)
...
# --------------------------------------------------------------------------------------------------------- #       
        ...
        if all(dist < 20 for dist in distances_to_target):  # 20 pixels as threshold
